...
class NEURON:
    ...
    def Update_Hidden_Or_Motor_Neuron(self, neurons, synapses):
        self.Set_Value(0.0)
        for synap in synapses.keys():
            if self.Get_Name() == synap[1]:
                self.Allow_Presynaptic_Neuron_To_Influence_Me(synapses[synap].Get_Weight(),
                                                              neurons[synap[0]].Get_Value())
        self.Threshold()

    def Allow_Presynaptic_Neuron_To_Influence_Me(self, weight, val):
        self.Add_To_Value(weight * val)
